...
describe("RedisChannel Interface", () => {
  ...
  it("should work", async () => {
        ...
        triggers: [
          {
            on: [
              {
                $: {
                  action: config.channels.listen[0],
                  contains: apiDescription.split(" ")[0],
                },
              },
            ],
          },
    ...
    return expectAsync(rcInterface.run())
      .toBeResolved()
      .then(() => expectAsync(rcInterface.run()).toBeResolved())
      .then(() => {
        console.log(fake.listenningChannels);
        expect(fake.listenningChannels)
          .withContext("listenningChannels to be equals")
          .toEqual(config.channels.listen);
        expect(fake.callbacks.message)
          .withContext("callbacks.message to be defined")
          .toBeDefined();
        expect(fake.callbacks.message.length)
          .withContext("callbacks.message.length")
          .toBe(1);
      })
      .then(() => {
        expectedValue = "";
