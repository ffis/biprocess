...
export class XMLJobSource implements JobSource {
  ...
  loadJobs(): Promise<JobElement[]> {
    return this.loadXML(
      this.parameters.referenceDirectory,
      this.parameters.jobsDirectory
    )
  ...
  private loadFromJobsDirectory(
    referenceDirectory: string,
    jobsDirectory: string
  ): Promise<string[]> {
    return fs
      .readdir(resolve(referenceDirectory, jobsDirectory))
      .then((files) => {
        return Promise.all(
          files
      ...
      .catch(() => {
        const message =
          resolve(__dirname, "..", "..", jobsDirectory) +
  ...
  private xml2jobs(xml: string | string[]): Promise<JobList> {
    ...
    if (typeof xml === "object" && Array.isArray(xml)) {
      return Promise.all(
        xml.map((data) => (parseXml(data) as unknown) as JobList)
      ).then((datas: JobList[]) =>
        datas.reduce(
          (p: JobList, c: JobList) => {
