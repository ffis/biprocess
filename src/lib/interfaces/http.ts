...
export class Commands {
  public help(jobs: JobElement[]): Promise<HelpCommandOutput> {
    const comms: HelpCommandOutput = Object.keys(
      supportedCommandsDescription
    ).map((s) => {
      return { command: "/" + s, description: supportedCommandsDescription[s] };
    });
    const options: HelpCommandOutput = jobs
      .map(
        (job): HelpCommandOutputItem => {
          return {
            command: job.$.key,
  ...
  isCommand(str: string): boolean {
    return (
      str.startsWith("/") &&
      Object.keys(AvailableCommands).indexOf(str.replace("/", "")) >= 0
    );
  }
  ...
  ): Promise<HelpCommandOutput | string> {
...
interface HTTPInterfaceOptionsInternal {
  config: ServerConfig;
  runEnteredCommand: (s: string) => Promise<void>;
  retrieveFromKey: (s: string) => Promise<string>;
  logger: Logger;
...
export class HTTPInterface implements BiprocessInterface {
  ...
  private parameters: HTTPInterfaceOptionsInternal;
  ...
  constructor(parameters: HTTPInterfaceOptions) {
    ...
    this.parameters = Object.assign({}, parameters, {
      logger: parameters.logger ? parameters.logger : console,
    });
    ...
    this.router
      .get("*", (req: Request, res: Response) => {
        if (req.originalUrl === "/") {
          ...
          this.parameters
            .retrieveFromKey(req.originalUrl)
            .then((val) => {
              if (val) {
      ...
      .post("*", (req: Request, res: Response) => {
        this.commands
          .run(req.originalUrl, this.jobs, this.parameters.runEnteredCommand)
          .then((value) => {
            res.json(value);
          })
          .catch((err) => {
  ...
  run(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server = this.app_.listen(
        this.parameters.config.port,
        this.parameters.config.bind,
        () => {
          this.parameters.logger?.log(
            "Listenning on port",
            this.parameters.config.port
          );
          resolve();
