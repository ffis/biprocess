...
export class Commands {
  public help(
    ...
    const options = jobs
      .map((job) => {
        return (
          job.$.key + (job.description ? EOL + "\t" + job.description : "")
        );
      })
      .concat(comms);
...
interface ReadLineInterfaceOptionsInternal {
  runEnteredCommand: (s: string) => Promise<void>;
  input: NodeJS.ReadableStream;
  output: NodeJS.WritableStream;
}
...
export class ReadLineInterface implements BiprocessInterface {
  ...
  private parameters: ReadLineInterfaceOptionsInternal;
  ...
  constructor(parameters: ReadLineInterfaceOptions) {
    ...
    this.parameters = Object.assign({}, parameters, {
      input: parameters.input ? parameters.input : process.stdin,
      output: parameters.output ? parameters.output : process.stdout,
    });
  }
  ...
  run(): Promise<void> {
    ...
    this.rl
      .on("line", (line) => {
        const command: Promise<void> = this.commands.run(
          line,
          this.jobs,
          this.parameters.runEnteredCommand,
