...
export function getInterfaces(
  ...
  const channels2subscribe: string[] =
    config.redis.channels && Array.isArray(config.redis.channels.listen)
      ? config.redis.channels.listen
      : [];
  if (channels2subscribe.length > 0) {
    const redisChannelInterface = new RedisChannelInterface({
      config: config.redis,
      createClient,
      runEnteredCommand,
    });
